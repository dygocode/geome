...
const MODELS = ["openai/gpt-oss-120b:free", "openai/gpt-oss-20b:free"];
...
async function callModel(model: string, messages: any[]): Promise<string> {
  const res = await fetch(`${OPENROUTER_API}/chat/completions`, {
    ...
    headers: {
      ...
      "X-Title": "Geome - ChatGPT Analysis",
    },
...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `Voce e o modelo de IA "ChatGPT". Analise quao bem a empresa "${companyName}" (${website}, segmento: ${segment}, localizacao: ${location}) apareceria nas suas respostas.

IMPORTANTE: Responda APENAS sobre a plataforma "ChatGPT". Nao mencione outras plataformas.

Retorne APENAS um objeto JSON (sem markdown, sem crases):
{
  "score": <numero 0-100>,
  "context": "<1-2 frases em portugues sobre como voce, como ChatGPT, descreveria/mencionaria esta empresa>",
...
LEMBRE-SE: Todo texto DEVE estar em portugues do Brasil. Responda SOMENTE com o JSON.`;
    ...
    if (!result) throw new Error(`ChatGPT analysis failed: ${lastError?.message}`);
